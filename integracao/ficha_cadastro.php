...
// Dados de integração com o SGE 
$nacionalidade  = isset($_SESSION['vch_nacionalidade']) ? $_SESSION['vch_nacionalidade'] : '';
$pais           = isset($_SESSION['vch_pais']) ? $_SESSION['vch_pais'] : '';
$uf             = isset($_SESSION['vch_uf']) ? $_SESSION['vch_uf'] : '';
$naturalidade   = isset($_SESSION['vch_naturalidade']) ? $_SESSION['vch_naturalidade'] : '';
?>
<div class="centr">
    <br>
    ...
    <form id="ficha" name="ficha" method="post" action="#">
        <div class="card-body">
            <div class="form-group">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <input required class="form-control" type="text" name="vch_nome" id="vch_nome" title="Informe o nome do aluno." value="<?php echo $nome ?>"  onKeyPress="mudarCorCampo('labelNome', 'vch_nome');return letras();" />
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <div class='input-group date' id='datetimepicker3'>
                            <input required value="<?php echo $data_nascimento ?>" class="form-control" type="text" name="sdt_nascimento" id="sdt_nascimento" title="Informe a data de nascimento do aluno." onchange="mudarCorCampo('labelDataNascimento', 'sdt_nascimento');testaIdade(this.value);">
                        </div>
                    </div>
                    <div class="col-md-4">
                        <label id="labelSexo" for="cp_sexo">Sexo:</label><span>*</span>
                        <select required class="browser-default custom-select" name="vch_sexo" id="cp_sexo" title="Selecione o sexo do aluno." onchange="mudarCorCampo('labelSexo', 'cp_sexo')">
                    ...
                    <div class="col-md-4">
                        ...
                        <select required class="custom-select" id="cp_serie" title="Selecione a série do aluno." name="vch_serie" onchange="mudarCorCampo('labelSerie', 'cp_serie')">
                            <option selected></option>
                            <?php
            ...
            <div class="form-group">
                <div class="row">
                    <div class="col-md-6">
                        <label id="labelNacionalidade" for="cp_nacionalidade">Nacionalidade:</label><span>*</span>
                        <select required class="browser-default custom-select" name="vch_nacionalidade" id="cp_nacionalidade" title="Selecione a nacionalidade do aluno." onchange="mudarCorCampo('labelNacionalidade', 'cp_nacionalidade');js_nacionalidade(this.value);">
                            <option selected></option>
                            <option value="1">BRASILEIRA</option>
                            <option value="2">BRASILEIRA NO EXTERIOR OU NATURALIZADO</option>
                            <option value="3">ESTRANGEIRA</option>
                        </select>
                        <script>
                            $('#cp_nacionalidade').val('<?php echo $nacionalidade; ?>');
                        </script>
                    </div>
                    <?php
                    $sql_pais = "select ed228_i_codigo,ed228_c_descr from escola.pais order by ed228_c_descr";
                    $result = pg_query($conn, $sql_pais);
                    $sql_pais = pg_fetch_all($result);
                    ?>
                    <div class="col-md-6">
                        <label id="labelPais" for="cp_pais">País:</label><span>*</span>
                        <select required class="custom-select" name="vch_pais" id="cp_pais" title="Selecione o país do aluno."onchange="mudarCorCampo('labelPais', 'cp_pais')">
                            <option selected></option>
                            <?php
                            foreach ($sql_pais as $pais_selec) {
                                echo '<option value=' . $pais_selec['ed228_i_codigo'] . '>' . $pais_selec['ed228_c_descr'] . "</option>";
                            }
                            ?>
                        </select>
                        <script>
                            $('#cp_pais').val('<?php echo $pais; ?>');
                        </script>
                    </div>
                </div>
            </div>
            <div class="form-group">
                <div class="row">
                    <!-- Inicio do Select UF -->  
                    <?php
                    $sql_uf = "select ed260_i_codigo,ed260_c_nome from escola.censouf order by ed260_c_nome";
                    $result = pg_query($conn, $sql_uf);
                    $sql_uf = pg_fetch_all($result);
                    ?>
                    <div class="col-md-6"> 
                        <label id="labelUf" for="cp_uf">UF de Nascimento:</label><span>*</span>
                        <select required class="custom-select" id="cp_uf" title="Selecione o Estado de nascimento do aluno." name="vch_uf" onchange="mudarCorCampo('labelUf', 'cp_uf');js_consultaNaturalidade(this.value);">
                            <option selected></option>
                            <?php
                            foreach ($sql_uf as $uf_selec) {
                                echo '<option value=' . $uf_selec['ed260_i_codigo'] . '>' . $uf_selec['ed260_c_nome'] . "</option>";
                            }
                            ?>
                        </select>
                        <script>
                            $('#cp_uf').val('<?php echo $uf; ?>');
                        </script>
                    ...
                    <!-- Inicio do Select UF --> 
                    <?php
                    $sql_naturalidade = "select ed261_i_codigo,ed261_c_nome from escola.censomunic order by ed261_c_nome";
                    $result = pg_query($conn, $sql_naturalidade);
                    $sql_naturalidade = pg_fetch_all($result);
                    ?>
                    <div class="col-md-6">
                        <label id="labelNaturalidade" for="cp_naturalidade">Naturalidade:</label><span>*</span>
                        <select required class="custom-select" id="cp_naturalidade" title="Selecione o município de nascimento do aluno." name="vch_naturalidade" onchange="mudarCorCampo('labelNaturalidade', 'cp_naturalidade')">
                            <option selected></option>
                                <?php
                                    foreach ($sql_naturalidade as $naturalidade_selec) {
                                        echo '<option value=' . $naturalidade_selec['ed261_i_codigo'] . '>' . $naturalidade_selec['ed261_c_nome'] . "</option>";
                                    }
                                ?>
                        </select>
                        <script>
                            $('#cp_naturalidade').val('<?php echo $naturalidade; ?>');
                        </script>

                    </div>
                </div>
            </div>
            <!-- Fim - Dados de Integração -->   
            <div class="row">
                <div class="col-md-12">
            ...
            <div class="form-group">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <input autocomplete="off" class="form-control" type="text" id="vch_orgaopublico" title="Informe o nome do orgão público que realiza o cadastro." name="vch_orgaopublico" id="vch_orgaopublico" value="<?php echo $vch_orgaopublico ?>" disabled onKeyPress="mudarCorCampo('labelOrgaoPublico', 'vch_orgaopublico');return letras();">
                    </div>
                </div>
            </div>
            <div class="form-group">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <input value="<?php echo "$nome_mae" ?>" class="form-control" type="text" name="vch_mae" id="vch_mae" title="Informe o nome da mãe do aluno." onKeyPress="mudarCorCampo('labelNomeMae', 'vch_mae');return letras();">
                    </div>
                </div>
            </div>
            <div class="form-group">
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <input autocomplete="off" class="form-control" type="text" name="vch_responsavel" title="Informe o nome do responsável legal do aluno." id="vch_responsavel" value="<?php echo $nome_responsavel ?>" onKeyPress="mudarCorCampo('labelNomeResponsavel', 'vch_responsavel');return letras();">
                    </div>
                </div>
            </div>
            <div class="form-group">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <input autocomplete="off" class="form-control" type="email" name="vch_email" id="vch_email" title="Informe o e-mail." value="<?php echo $vch_email ?>">
                    </div>
                </div>
            </div>
            <div class="form-group">
                <div class="row">
                    <div class="col-md-5">
                        ...
                        <input value="<?php echo $cpf ?>" class="form-control" type="text" name="vch_cpf" id="vch_cpf" title="Informe o CPF." onKeyPress="mudarCorCampo('labelCpf', 'vch_cpf')">
                    </div>
                </div>
            </div>
            <div class="row">
    ...
    <script>
        function js_nacionalidade(nacionalidade){
            if (nacionalidade == 1) { 
                $('#cp_pais').val(10);              // Nacionalidade Brasileira; 
                $('#cp_pais').attr("disabled",false);
                $('#cp_uf').attr("disabled",false);
                $('#cp_naturalidade').attr("disabled",false);
            }else if(nacionalidade == 2){
                $('#cp_pais').val(20);               // Outra nacionalidade; 
                $('#cp_pais').attr("disabled",true);
                $('#cp_uf').attr("disabled", true);
                $('#cp_uf').val('');
                $('#cp_naturalidade').attr("disabled",true);
                $('#cp_naturalidade').val('');
            }else{
                $('#cp_pais').val(''); 
                $('#cp_pais').attr("disabled",false);
        ...
        // Início função - Retorna a lista de cidades
        
        function js_consultaNaturalidade(unidadeFederacao) {
        $('#loading').show();
        $.ajax({
            url: "pesq.php",
            type: 'get',
            data: {
                codigo: unidadeFederacao,
                funcao: 'carregar_naturalidade'
            },
            beforeSend: function () {}
            })
                .done(function (msg) {
        ...
        // Fim da função - Consulta Naturalidade. 
