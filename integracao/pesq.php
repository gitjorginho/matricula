...
if (isset($_GET['funcao'])) {
    ...
    switch ($_GET['funcao']) {
        ...
        case 'carregar_autocomplete':
            ...
            inner join cadastro.ruasbairrocep rbc on rbc.j32_ruasbairro = rb.j16_codigo
            inner join cadastro.bairro b on b.j13_codi = rb.j16_bairro 
            where j14_nome ilike '%{$ender}%' or j13_descr ilike '%{$ender}%'";
            ...
            foreach ($enderecos as $endereco) {
                echo "<option value='{$endereco['j14_codigo']}-{$endereco['j13_codi']}'>" . $endereco['j14_nome'] . " - " . $endereco['j13_descr'] . "</option>";
            }
            break;
            
            case 'carregar_naturalidade':
            ...
            $sql_unidade = "
                ...
                from 
                    escola.censomunic 
                where 
                    ed261_i_censouf = {$codigo_unidade} 
                order by
                    ed261_c_nome
            ";
              
            $result = pg_query($conn, $sql_unidade);
            $unidadeFederacao = pg_fetch_all($result);

            echo '<option></option>';
            foreach ($unidadeFederacao as $unidadeFederacaoSelect) {
                echo '<option value='.$unidadeFederacaoSelect['ed261_i_codigo'].'>'.$unidadeFederacaoSelect['ed261_c_nome']."</option>";
            }
